...
test.describe('navigation', () => {
  ...
  test('header appears on home page', async ({page}) => {
    ...
    await expect(page.locator('li').filter({hasText: 'Contact'})).toBeVisible();
  ...
  test('content section one appearance', async ({page}) => {
    ...
    await expect(page.getByRole('heading', { name: 'Unlock Your Financial Potential with PennyWise' })).toBeVisible();
    await expect(page.getByText('Welcome to PennyWise, your')).toBeVisible();
    await expect(page.getByRole('link', { name: 'Explore PennyWise' })).toBeVisible();
  });


  test('content section two appearance', async ({page}) => {
    await expect(page.getByText('Features')).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Unlock Your Financial Potential', exact: true })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Budget Analytics' })).toBeVisible();
  ...
  test('footer appearance', async ({page}) => {
    ...
    await expect(page.getByRole('contentinfo')).toContainText('© 2024 PennyWise™ - By @CheezyDevs');
